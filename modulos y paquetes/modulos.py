"""
el archivo main es parte de este bloque
contiene mas información
Modulos y paquetes
un modulo es un archivo que contiene codigo python
"""
def suma(val1, val2):
    return val1 + val2
...
if __name__ == '__main__':
    print("soy un mensaje de modulos")
else:
...
class MyModulo:
  ...
  def metodo(self):
    ...
    pass
Cómo observamos nuestro script cuenta con tres funciones, una clase y su
 correspondiente documentación.

El primer comentario es de suma importancia ya que es aquí donde describiremos al
módulo, que es lo que hace,
que es lo que no hace, cómo funciona, cómo usarlo, entre otras anotaciones que
necesitemos colocar, posteriormente podemos colocar algunos metadatos. Los metadatos
más comunes son los ocho que podemos observar, sin embargo, si nosotros lo necesitamos
 podemos agregar los nuestros.

Para que pueda considerarse un metadato debemos de colocar doble guión bajo al
inicio y doble guión bajo al final.

Una vez hayamos documentado el script podemos pasar a documentar puntualmente
las funciones y las clases.

Si requerimos visualizar la documentación de un módulo basta con un utilizar
 la función help.
